...
function AIInterviewPage() {
  ...
  const handleFetchAIContent = async (options = {}) => {
    ...
    try {
      ...
      // 2. Set Edge Cases
      if (data.edgeCases) {
        console.log('📦 Received Edge Cases from API:', data.edgeCases);
        ...
        if (data.edgeCases.length > 0) {
          ...
          console.log('🔄 Converted Edge Cases for Console:', jsonTestFormat);
          setAutoGeneratedInput(jsonTestFormat);
        } else {
          console.warn('⚠️ Edge cases array is empty!');
        }
      } else {
        console.warn('⚠️ No edgeCases field in API response:', data);
      }
